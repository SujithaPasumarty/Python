...
def angle_to_vector(tita):
    return [math.cos(tita),math.sin(tita)]
def distance_between_points(p,q):
    return math.sqrt((p[0]-q[0])**2-(p[1]-q[1])**2)
def key_down_hadler(key):
...
def draw(canvas):
    ...
    canvas.draw_text(str(lives), [50, 100], 20, "White")
...
def rock_spawner():
    ...
    rock_vel = [(random.random() * 0.5 )- 0.2, (random.random() * 0.5) - 0.2]
...
frame.set_keydown_handler(key_down_hadler)
frame.set_keyup_handler(key_up_hadler)
